...
.card button:hover {
  background: #d97706;
    ...
    this.customJS = `// أضف شيفرات جافاسكريبت التفاعلية هنا
console.log("تم تحميل الصفحة المصممة بنجاح!");`;
  ...
  syncEditorToCanvas() {
    ...
    if (this.currentLanguage === 'html') {
      try {
        ...
        if (parserError) {
          console.warn('HTML parse error, waiting for complete tag...');
          return; 
        }
  ...
  renderBlocksList(searchVal = '', filterCat = 'all') {
  ...
  getLegacyBlockMapping(blockId) {
    const core = typeof VisualLogicCore !== 'undefined' ? VisualLogicCore : null;
    if (!core) return null;
  ...
  revealEditorRange(start, end) {
    ...
    } catch (err) { /* caret placement is cosmetic only */ }
