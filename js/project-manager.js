...
class ProjectManager {
  ...
  updateLinkedPreview() {
    this.ensureLinkedStyleTag();
    this.linkedStyleTag.textContent = this.getLinkedContent('css');
  ...
  async saveProjectToFolder() {
    ...
    try {
      ...
      for (const file of this.project.files) {
        const output = file.kind === 'html' ? this.composeHtmlPage(file, false) : file;
        await this.writeFileToDirectory(directory, file.path, output);
      }
      this.persistWorkspace();
      this.render();
      this.notice(`تم حفظ ${this.project.files.length} ملف داخل مجلد «${directory.name}»`);
    } catch (error) {
  ...
  exportProject() {
    ...
    const project = this.project;
    if (!project) return;
    const outputs = project.files.map(file => file.kind === 'html' ? this.composeHtmlPage(file, false) : file);
  ...
  composeHtmlPage(page, inlineLinked) {
    const project = this.project;
    ...
    if (this.app.styleEngine && typeof this.app.styleEngine.getExportCSS === 'function') {
      try { inlineCss = this.app.styleEngine.getExportCSS(inlineCss); } catch (error) { /* Keep raw CSS. */ }
    }
    let inlineJs = String(page.js || '').replace(/^\s*\/\/ OSOOS_COMPONENT_DATA:.*(?:\r?\n|$)/gm, '');
    const linkedCss = [];
    const linkedJs = [];
...
${scriptLinks}
  ...
  fallbackCopy(value, onSuccess) {
    ...
    textarea.select();
    try { document.execCommand('copy'); onSuccess(); } catch (error) { this.notice(value); }
    textarea.remove();
  }
