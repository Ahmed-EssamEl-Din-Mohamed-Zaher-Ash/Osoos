...
class HistoryState {
  constructor(canvasHTML, customCSS, customJS, selectedElementId) {
...
class HistoryManager {
  ...
  saveState(reason = '') {
    ...
    const canvasHTML = typeof this.app.getPersistentCanvasHTML === 'function'
      ? this.app.getPersistentCanvasHTML()
      : canvas.innerHTML;
    const customCSS = this.app.editor.customCSS;
    const customJS = this.app.editor.customJS;
    const selectedElementId = this.app.selectedElement ? this.app.selectedElement.id : null;
    
    // Compare with the last state to prevent duplicate consecutive states
    if (this.undoStack.length > 0) {
      const last = this.undoStack[this.undoStack.length - 1];
      if (last.canvasHTML === canvasHTML && 
          last.customCSS === customCSS && 
          last.customJS === customJS && 
          last.selectedElementId === selectedElementId) {
        return; // Duplicate state, skip
      }
    }
    
    const state = new HistoryState(canvasHTML, customCSS, customJS, selectedElementId);
    this.undoStack.push(state);
    
    // Cap size at maxStates (20)
  ...
  undo() {
    if (this.debounceTimeout) {
      clearTimeout(this.debounceTimeout);
      this.debounceTimeout = null;
      this.saveState('Forced Debounce Save');
    }

    if (this.undoStack.length <= 1) {
      if (typeof this.app.showToastNotice === 'function') {
    ...
    // Pop current state to push into redoStack
    const currentState = this.undoStack.pop();
    this.redoStack.push(currentState);
    
    // Get target state to restore
    const targetState = this.undoStack[this.undoStack.length - 1];
    /* استثناء داخل restoreState كان يترك isRestoring = true فيتعطل الـ history للأبد */
    try {
  ...
  restoreState(state) {
    ...
    // 1. Restore canvas content
    canvas.innerHTML = state.canvasHTML;
    
    // 2. Restore editor code
    this.app.editor.customCSS = state.customCSS;
    this.app.editor.customJS = state.customJS;
    
    // Update code editor value
    this.app.editor.refreshEditorContent();
    
    // Re-inject custom CSS styles
    this.app.applyCustomCSS(state.customCSS);
    
    // Reattach event listeners to all newly restored elements in canvas
    this.app.reattachCanvasListeners();
    
    // 3. Restore selection
    let selectedEl = null;
    if (state.selectedElementId) {
      /* معرف يبدأ برقم كان يرمي SyntaxError ويعطّل الـ history */
      const escapedId = typeof CSS !== 'undefined' && CSS.escape ? CSS.escape(state.selectedElementId) : state.selectedElementId;
      try { selectedEl = canvas.querySelector(`#${escapedId}`); } catch (error) { selectedEl = null; }
    }
    
    // Temporarily select
    this.app.selectElement(selectedEl);
    
    // 4. Update all UI representations
    this.app.domTree.render();
    this.app.properties.updatePanelFor(selectedEl);
    if (selectedEl) {
...
class WebBuilderApp {
  ...
  setupHighlighterButtons() {
    ...
    // JS link quick menu (build interaction / link to another element / custom logic ...)
    const jsLinkBtn = document.getElementById('bubble-js-link');
    if (jsLinkBtn) {
      jsLinkBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        if (this.editor && typeof this.editor.toggleJsLinkMenu === 'function') {
          this.editor.toggleJsLinkMenu();
        } else {
          const btnJs = document.getElementById('tab-btn-js');
          if (btnJs) btnJs.click();
        }
      });
    }
  ...
  sanitizeRestoredHtml(html) {
    ...
    template.content.querySelectorAll('*').forEach(node => {
      Array.from(node.attributes || []).forEach(attribute => {
        ...
        else if ((name === 'href' || name === 'src' || name === 'xlink:href') && /^\s*javascript:/i.test(value)) node.removeAttribute(attribute.name);
      });
    });
...
<body>
  ...
  openFinalPreview() {
    ...
    frame.srcdoc = this.buildExportDocument();
  ...
  setupTabSwitcher() {
    ...
    if (btnJs) {
      btnJs.addEventListener('click', () => {
        ...
        if (panelTitle) panelTitle.textContent = 'المنطق – كتل جافاسكريبت';
