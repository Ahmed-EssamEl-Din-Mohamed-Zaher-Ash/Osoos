...
import { getElementInfo } from '../../data/htmlElements.js';
...
class DOMTreeManager {
  ...
  handlePaletteDragOver(event) {
    ...
    const placement = this.resolveDropPlacement(
      ...
      target,
      event.clientY,
      (candidate) => this.isPaletteDropAllowed(tag, target, candidate)
    );
    const { position, allowed } = placement;
  ...
  /*
   * A pointer position expresses intent, but it should not turn most of a row
   * into a dead zone when HTML constraints reject that exact interpretation.
   *
   * Examples:
   * - Dropping <li> near an edge of <ul> first reads as "beside ul". Since an
   *   li cannot live in ul's parent, fall back to "inside ul".
   * - Dropping <li> over the middle of another <li> first reads as "inside
   *   li". Fall back to the nearest sibling edge inside their existing ul.
   */
  resolveDropPlacement(wrapper, target, clientY, isAllowed) {
    const preferred = this.getDropPosition(wrapper, target, clientY);
    const rect = wrapper.getBoundingClientRect();
    const height = Math.max(rect.height, 1);
    const ratio = Math.min(1, Math.max(0, (clientY - rect.top) / height));
    const nearestEdge = ratio < 0.5 ? 'before' : 'after';
    const oppositeEdge = nearestEdge === 'before' ? 'after' : 'before';

    const candidates = preferred === 'inside'
      ? ['inside', nearestEdge, oppositeEdge]
      : [preferred, 'inside', oppositeEdge];
    const uniqueCandidates = [...new Set(candidates)];

    for (const position of uniqueCandidates) {
      if (isAllowed(position)) return { position, allowed: true };
    }

    return { position: preferred, allowed: false };
  }
