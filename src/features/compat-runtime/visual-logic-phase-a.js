...
(function () {
  ...
  /* E1.3.3 — المسار البسيط: تسميات عربية بلغة المعالج المبسّط نفسها
     (متى يبدأ؟ / ماذا سيحدث؟ / بشرط أن…) + الخطوات المتقدمة تحت زر «متقدم» */
  const E13_STEP_LABELS_AR = ['متى يبدأ؟', 'القراءة', 'بشرط أن…', 'ماذا سيحدث؟', 'البيانات', 'الدوال', 'أدوات متقدمة', 'راجع واحفظ'];
  const E13_ADVANCED_STEPS = [5, 6, 7];
  const DESTINATION_LABELS = {
    reads: 'القراءة', conditions: 'الشروط', actions: 'الإجراءات', state: 'البيانات', functions: 'الدوال'
  };
  ...
  proto.getInteractionDefinitions = function () {
    if (!Array.isArray(this.interactionDefinitions)) this.interactionDefinitions = [];
    return core.clone(this.interactionDefinitions);
  };

  proto.upsertInteractionDefinition = function (input) {
    const definition = core.normalizeDefinition(input);
    if (!Array.isArray(this.interactionDefinitions)) this.interactionDefinitions = [];
    const index = this.interactionDefinitions.findIndex(item => item && item.id === definition.id);
    if (index === -1) this.interactionDefinitions.push(core.clone(definition));
    else this.interactionDefinitions[index] = core.clone(definition);
    return definition;
  };

  proto.removeInteractionDefinition = function (id) {
    if (!Array.isArray(this.interactionDefinitions)) this.interactionDefinitions = [];
    this.interactionDefinitions = this.interactionDefinitions.filter(item => item && item.id !== id);
  };

  /* ترحيل صامت وآمن للمشاريع القديمة: نستخرج التعريف من التعليق المشفّر مرة
     واحدة، نحفظه كبيانات مشروع منفصلة، ثم نستبدل البلوك بكود حديث مقروء. */
  proto.setInteractionDefinitions = function (definitions, options) {
    const settings = options || {};
    const supplied = Array.isArray(definitions) ? definitions : [];
    const byId = new Map();
    supplied.filter(Boolean).forEach(item => {
      const normalized = core.normalizeDefinition(item);
      byId.set(normalized.id, normalized);
    });

    const legacyLinks = core.parseVisualLinks(this.customJS || '');
    legacyLinks.forEach(link => {
      if (!byId.has(link.id)) byId.set(link.id, core.normalizeDefinition(link));
    });
    this.interactionDefinitions = Array.from(byId.values()).map(item => core.clone(item));

    if (settings.migrateLegacy === false || !legacyLinks.length) return false;
    const lines = String(this.customJS || '').split(/\r?\n/);
    legacyLinks
      .slice()
      .sort((a, b) => b.startIndex - a.startIndex)
      .forEach(link => {
        const definition = byId.get(link.id) || link;
        lines.splice(
          link.startIndex,
          link.endIndex - link.startIndex + 1,
          ...core.generateBlock(definition).split('\n')
        );
      });
  ...
  proto.parseVisualLinks = function () {
    if (!Array.isArray(this.interactionDefinitions)) this.interactionDefinitions = [];
    return core.parseVisualLinks(this.customJS, this.interactionDefinitions);
  };

  proto.getExportJavaScript = function (customJS, definitions) {
    const source = customJS === undefined ? this.customJS : customJS;
    const stored = definitions === undefined ? this.interactionDefinitions : definitions;
    const lines = String(source || '').split(/\r?\n/);
    core.parseVisualLinks(source, Array.isArray(stored) ? stored : [])
      .slice()
      .sort((a, b) => b.startIndex - a.startIndex)
      .forEach(link => {
        lines.splice(
          link.startIndex,
          link.endIndex - link.startIndex + 1,
          ...core.generateExecutable(link).split('\n')
        );
      });
    return lines.join('\n')
      .replace(/^\s*\/\/\s*(?:@(?:end-)?osoos-interaction|OSOOS_VISUAL_LINK_(?:START|END)).*$(?:\r?\n)?/gm, '')
      .replace(/^\s*\/\/\s*(?:OSOOS_LOGIC_DATA|SOURCE_ID|TARGET_ID|EVENT|MODE|PARAMS):.*$(?:\r?\n)?/gm, '')
  ...
  /* أسماء ودّية: «قائمة «المهام»» بدل ul#ul-b0n2wuff — والمعرّف التقني يبقى في التلميح */
  function friendlyE1Label(element) {
    const hubCoreApi = typeof OsoosInteractionHubCore !== 'undefined' ? OsoosInteractionHubCore : null;
    if (hubCoreApi && element) return hubCoreApi.friendlyElementLabel(hubCoreApi.describeDomElement(element));
    return element ? `${element.tagName.toLowerCase()}${element.id ? '#' + element.id : ''}` : '';
  }

  proto.getVisualElementLabel = function (id) {
    if (!id) return 'لم يُحدد بعد';
    const element = document.getElementById(id);
    return element ? friendlyE1Label(element) : `عنصر مفقود (#${id})`;
  };
  proto.getCanvasElementOptions = function (selectedId) {
    ...
    const labels = elements.map(element => friendlyE1Label(element));
    const counts = labels.reduce((all, label) => { all[label] = (all[label] || 0) + 1; return all; }, {});
    html += elements.map((element, index) => {
      const label = counts[labels[index]] > 1 ? `${labels[index]} — ${element.tagName.toLowerCase()}#${element.id}` : labels[index];
      return opt(element.id, label, selectedId);
    }).join('');
    return html;
  };
    ...
    overlay.innerHTML = `
      <div class="vl-popup vl-universal e1-dialog" role="dialog" aria-modal="true">
        ...
        <div class="e1-source-strip" id="e1-source-strip">
          ...
          <button type="button" class="vl-link-button" id="e1-change-source">تغيير العنصر</button>
        </div>
  ...
  proto.renderE1Builder = function () {
    ...
    if (!recipeMode) holder.insertAdjacentHTML('afterbegin', this.renderE1LiveSummary() + this.renderE13FlowBanner(this.e1CurrentStep) + this.renderE11StepOverview());
    else holder.insertAdjacentHTML('afterbegin', this.renderE1LiveSummary());
    this.bindE1ContentEvents();
    this.previewLinkArrow = null;
    this.updateVisualLinkArrows();
  };
  ...
  proto.renderE11StepOverview = function () {
    ...
    return `<div class="e11-step-overview"><span>لديك:</span><b>${definition.reads.length} قراءة</b><b>${definition.conditions.length} شرط</b><b>${actions.length} إجراء</b><b>${state.length} متغير</b><b>${(definition.functions || []).length} دالة</b><b>${(definition.advancedOperations || []).length} عملية متقدمة</b></div>`;
  };

  /* الجملة الحية أعلى المنشئ: التفاعل كله في سطر عربي واحد يتحدث مع كل تعديل */
  proto.renderE1LiveSummary = function () {
    const definition = this.visualLinkDraft;
    if (!definition || definition.builderMode === 'function' || definition.builderMode === 'component' || definition.componentType) return '';
    const hubCoreApi = typeof OsoosInteractionHubCore !== 'undefined' ? OsoosInteractionHubCore : null;
    let sentence;
    if (hubCoreApi) {
      sentence = hubCoreApi.summarizeDefinition(definition, {
        eventLabels: EVENT_LABELS,
        labelFor: id => (id ? `«${this.getVisualElementLabel(id)}»` : 'العنصر'),
        actionPhrase: action => {
          const label = core.ACTION_TYPES[action.type] || action.type;
          const targetId = action.targetId || (action.target && action.target.id);
          if (!targetId || targetId === definition.sourceId) return label;
          return `${label} — «${this.getVisualElementLabel(targetId)}»`;
        },
        conditionPhrase: condition => {
          const read = (definition.reads || []).find(item => item && item.name === condition.left);
          const target = read ? `«${this.getVisualElementLabel(read.elementId)}»` : String(condition.left || 'القيمة');
          if (condition.operator === 'notEmpty') return `بشرط ألا يكون ${target} فارغًا`;
          const operatorLabel = (core.CONDITION_LABELS || {})[condition.operator] || condition.operator;
          return `بشرط أن ${target} ${operatorLabel}${condition.right ? ` «${condition.right}»` : ''}`;
        }
      });
    } else {
      sentence = this.getE1Summary(definition);
    }
    return `<div class="e1-live-summary" aria-live="polite"><i class="fas fa-wand-magic-sparkles"></i><span>${esc(sentence)}</span></div>`;
  };
  ...
  proto.saveVisualLinkFromPopup = function () {
    ...
    const previousId = this.activeVisualLink && this.activeVisualLink.existingId;
    const existing = !!previousId; this.flushPendingHistoryBeforeVisualLink();
    this.customJS = renameResult.customJS;
    if (previousId && previousId !== validation.definition.id) this.removeInteractionDefinition(previousId);
    this.upsertInteractionDefinition(validation.definition);
  ...
  proto.deleteVisualLink = function (id) {
    ...
    this.removeInteractionDefinition(id);
    if (this.currentLanguage === 'js') { this.textarea.value = this.customJS; this.updateLineNumbers(); }
  ...
  proto.tryComponentFromPopup = function () {
    ...
    if (this._vlTrialMessageHandler) {
      window.removeEventListener('message', this._vlTrialMessageHandler);
    }
    this._vlTrialMessageHandler = errorHandler;
    window.addEventListener('message', errorHandler);
    
    overlay.querySelector('iframe').srcdoc = documentHTML;
    overlay.querySelector('#e1-trial-cancel').addEventListener('click', () => this.closeVisualLinkTrial());
    overlay.querySelector('#e1-trial-edit').addEventListener('click', () => this.closeVisualLinkTrial());
