...
(function () {
  ...
  const ACTION_LABELS = {
    ...
    'bubble-demo': 'قواعد التفاعل: لو / إذن / وإلا',
  ...
  class ResponsiveSelectionToolbar {
    ...
    buildGroups() {
      ...
      const primaryIds = ['bubble-layers-btn', 'bubble-demo'];
      ...
      ['bubble-select-parent', 'bubble-move', 'bubble-snap', 'bubble-reset-move', 'bubble-delete'].forEach(id => {
        const button = this.action(id);
        if (!button) return;
    ...
    enhanceActions() {
      ...
      const demoButton = this.action('bubble-demo');
      if (demoButton) {
        demoButton.setAttribute('aria-haspopup', 'dialog');
