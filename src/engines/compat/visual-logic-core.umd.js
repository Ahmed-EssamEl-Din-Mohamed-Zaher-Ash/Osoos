...
(function (root, factory) {
  ...
  const E1_READ_TYPES = {
    ...
    element: schemaEntry('العنصر نفسه', 'element', ['elementId'], { sourceKinds: ['element'], resultHint: 'element' }),
  ...
  const E1_ACTION_TYPES = {
    ...
    /* عمليات القوائم: كل نوع هنا يقابل سطر JavaScript واحدًا يكتبه المبرمج بيده */
    arrayPop: 'حذف آخر عنصر من Array', arrayShift: 'حذف أول عنصر من Array', arrayUnshift: 'إضافة في بداية Array', arrayClear: 'تفريغ Array',
    arraySort: 'ترتيب Array', arrayReverse: 'عكس ترتيب Array', arrayUnique: 'إزالة المكرر من Array',
    arrayJoin: 'دمج Array في نص', arraySlice: 'قص جزء من Array', arrayFilter: 'تصفية Array بشرط', arrayMap: 'تحويل كل عناصر Array',
    arrayLength: 'عدد عناصر Array', arraySum: 'مجموع أرقام Array', arrayIndexOf: 'موضع قيمة في Array',
    arrayItem: 'قراءة عنصر من Array بموضعه',
    /* عمليات النصوص: قص، تقسيم، استبدال… بلا كتابة كود */
    stringSlice: 'قص جزء من نص', stringSplit: 'تقسيم نص إلى Array', stringReplace: 'استبدال داخل نص',
    stringCase: 'تغيير حالة الأحرف', stringTrim: 'إزالة المسافات الزائدة', stringLength: 'عدد حروف نص',
    stringConcat: 'دمج نصين', stringNumber: 'تحويل نص إلى رقم',
    /* بطاقة تُترجَم إلى كود جاهز، فتحتاج اسمًا يظهر في تعليق الخطوة */
    renderList: 'عرض قائمة البيانات في عنصر',
    /* الكونسول: نافذة المطوّر لمتابعة القيم أثناء التنفيذ */
    consoleLog: 'طباعة في الكونسول', consoleTable: 'عرض Array كجدول في الكونسول', consoleClear: 'مسح الكونسول',
  ...
  const E1_CONDITION_OPERATORS = {
    ...
    hasClass: 'يحتوي Class', notHasClass: 'لا يحتوي Class', visible: 'ظاهر', hidden: 'مخفي', enabled: 'مفعّل', disabled: 'معطل', checked: 'محدد', focused: 'عليه Focus', hasChildren: 'يحتوي أبناء', hasAttribute: 'Attribute موجود',
  ...
  Object.keys(E1_ACTION_TYPES).forEach(type => {
    ...
    /* الكونسول والنصوص لهما تصنيفهما الخاص، ويُفحصان أولاً حتى لا تلتقطهما
       التعبيرات العامة أدناه (مثل append داخل اسم غير متعلق بالعناصر). */
    if (/^console/.test(type)) category = 'console';
    else if (/^string/.test(type)) category = 'text';
    else if (/Style|Color|Background|Width|Height|Opacity|Transform|Class/.test(type)) category = 'style';
    ...
    /* بطاقات القوائم والنصوص والكونسول تعمل على قيم ومتغيرات لا على عناصر DOM */
    const targetless = /^((array|string|console)[A-Z]\w*|branch|loop|setVariable|(local|session)Storage(Set|Remove)|alert|confirm|prompt|openUrl|redirect|clipboard|print|setTimeout|setInterval|clearInterval|delayedAction|callFunction|custom)$/.test(type);
  ...
  const ACTION_FIELD_SCHEMAS = {
    ...
    setVariable: ['variableName', 'valueSource'], incrementVariable: ['variableName', 'step', 'display', 'cycleArray'], decrementVariable: ['variableName', 'step', 'display', 'cycleArray'], toggleBoolean: ['variableName', 'className'], arrayPush: ['arrayName', 'valueSource'], arrayRemove: ['arrayName', 'index', 'count'], arraySet: ['arrayName', 'index', 'valueSource'],
    arrayPop: ['arrayName', 'resultName'], arrayShift: ['arrayName', 'resultName'], arrayUnshift: ['arrayName', 'valueSource'], arrayClear: ['arrayName'],
    arraySort: ['arrayName', 'direction', 'numeric'], arrayReverse: ['arrayName'], arrayUnique: ['arrayName', 'resultName'],
    arrayJoin: ['arrayName', 'separator', 'resultName'], arraySlice: ['arrayName', 'start', 'end', 'resultName'],
    arrayFilter: ['arrayName', 'operator', 'compareValue', 'resultName'], arrayMap: ['arrayName', 'transform', 'template', 'resultName'],
    arrayLength: ['arrayName', 'resultName'], arraySum: ['arrayName', 'resultName'], arrayIndexOf: ['arrayName', 'valueSource', 'resultName'],
    arrayItem: ['arrayName', 'index', 'resultName'],
    stringSlice: ['valueSource', 'start', 'end', 'resultName'], stringSplit: ['valueSource', 'separator', 'resultName'],
    stringReplace: ['valueSource', 'search', 'replacement', 'all', 'resultName'], stringCase: ['valueSource', 'mode', 'resultName'],
    stringTrim: ['valueSource', 'resultName'], stringLength: ['valueSource', 'resultName'],
    stringConcat: ['valueSource', 'suffix', 'separator', 'resultName'], stringNumber: ['valueSource', 'resultName'],
    consoleLog: ['valueSource', 'label', 'level'], consoleTable: ['arrayName'], consoleClear: [],
  ...
  function generateAdvancedPhaseV3(definition, destination, options) {
    const traceSteps = Boolean(options && options.traceSteps);
    return orderedAdvancedOperations(definition, destination)
      .map(operation => markStep(`decl:${operation.id}`, generateAdvancedOperation(operation, definition, options), traceSteps))
      .filter(Boolean)
      .join('\n');
  }
  ...
  /* ═══════════ أسماء مقروءة للكود المولّد ═══════════
     الهدف: يقرأ المستخدم كودًا يشبه ما يكتبه المبرمج بيده.
     بدل  actionTarget2 / readElement1 / demoValue
     يظهر  notesList   / noteInput     / noteInputValue
     الاشتقاق من معرّف العنصر نفسه: "notes-list" ← notesList
     والمعرّفات المولّدة تلقائيًا مثل "ul-b0n2wuff" تصبح list1 / list2. */
  const READABLE_TAG_NOUNS = {
    ul: 'list', ol: 'list', li: 'item', dl: 'list', button: 'button', a: 'link',
    input: 'input', textarea: 'textarea', select: 'select', option: 'option', form: 'form',
    img: 'image', picture: 'image', video: 'video', audio: 'audio', iframe: 'frame',
    p: 'text', span: 'label', strong: 'text', em: 'text', small: 'text', code: 'code',
    div: 'box', section: 'section', article: 'article', aside: 'aside', main: 'main',
    header: 'header', footer: 'footer', nav: 'nav', figure: 'figure',
    table: 'table', thead: 'tableHead', tbody: 'tableBody', tr: 'row', td: 'cell', th: 'cell',
    h1: 'heading', h2: 'heading', h3: 'heading', h4: 'heading', h5: 'heading', h6: 'heading',
    label: 'label', canvas: 'canvas', svg: 'icon', hr: 'divider', br: 'lineBreak'
  };

  const RESERVED_CODE_NAMES = new Set([
    'state', 'event', 'sourceElement', 'controller', 'previousController', 'window', 'document',
    'item', 'index', 'items', 'value', 'element', 'target', 'targetElement', 'actionTarget',
    'copySource', 'storage', 'raw', 'error', 'arguments', 'eval', 'globalThis', 'console',
    'break', 'case', 'catch', 'class', 'const', 'continue', 'debugger', 'default', 'delete',
    'do', 'else', 'export', 'extends', 'finally', 'for', 'function', 'if', 'import', 'in',
    'instanceof', 'let', 'new', 'return', 'super', 'switch', 'this', 'throw', 'try', 'typeof',
    'var', 'void', 'while', 'with', 'yield', 'async', 'await', 'null', 'true', 'false', 'undefined'
  ]);

  function camelFromId(rawId) {
    const parts = String(rawId || '').split(/[^a-zA-Z0-9]+/).filter(Boolean);
    if (!parts.length) return '';
    const head = parts[0].replace(/^[0-9]+/, '').toLowerCase();
    if (!head) return '';
  ...
  /* معرّف مولّد آليًا = بادئة وسم معروف + لاحقة عشوائية فيها أرقام وحروف */
  function autoGeneratedIdNoun(rawId) {
    const match = String(rawId || '').match(/^([a-zA-Z]+)[-_]([a-zA-Z0-9]{6,})$/);
    if (!match) return '';
    const head = match[1].toLowerCase();
    const tail = match[2];
    if (!Object.prototype.hasOwnProperty.call(READABLE_TAG_NOUNS, head)) return '';
    if (!/[0-9]/.test(tail) || !/[a-zA-Z]/.test(tail)) return '';
    return READABLE_TAG_NOUNS[head];
  }

  function createNameScope(reservedNames) {
    const used = new Set(RESERVED_CODE_NAMES);
    (reservedNames || []).forEach(name => { if (name) used.add(String(name)); });
    const counters = Object.create(null);
    const scope = {
      isTaken(name) { return used.has(name); },
      reserve(name) { if (name) used.add(name); return name; },
      /* اسم فريد: الاسم المطلوب، ثم بديل وصفي، ثم ترقيم */
      unique(base, alternate) {
        const first = /^[a-zA-Z_$][\w$]*$/.test(base || '') ? base : '';
        if (first && !used.has(first)) { used.add(first); return first; }
        const second = alternate && /^[a-zA-Z_$][\w$]*$/.test(alternate) ? alternate : '';
        if (second && !used.has(second)) { used.add(second); return second; }
        return scope.sequential(first || second || 'element');
      },
      /* أسماء متسلسلة مثل list1 / list2 للمعرّفات المولّدة آليًا */
      sequential(base) {
        const stem = /^[a-zA-Z_$][\w$]*$/.test(base || '') ? base : 'element';
        let counter = counters[stem] || 1;
        while (used.has(stem + counter)) counter += 1;
        counters[stem] = counter + 1;
        const name = stem + counter;
        used.add(name);
        return name;
      }
    };
  ...
  function elementCodeName(elementId, scope, fallbackNoun) {
    const noun = autoGeneratedIdNoun(elementId);
    if (noun) return scope.sequential(noun);
    const camel = camelFromId(elementId);
    if (!camel) return scope.sequential(fallbackNoun || 'element');
    return scope.unique(camel, camel + 'Element');
  }

  /* اسم مقروء لقيمة تُقرأ من عنصر:  note-input ← noteInputValue
     والمعرّفات المولّدة آليًا مثل input-b0n2wuff ← inputValue.
     تُستعمل من interaction-demo.js حتى لا يظهر اسم آلي مثل demoValue. */
  function readableValueName(elementId, fallback) {
    const noun = autoGeneratedIdNoun(elementId);
    if (noun) return noun + 'Value';
    const camel = camelFromId(elementId);
    if (!camel) return fallback || 'fieldValue';
    return camel + 'Value';
  }

  function definitionReservedNames(definition) {
    const names = [];
    (definition.state || []).forEach(variable => names.push(safeIdentifier(variable.name, 'value')));
    (definition.reads || []).forEach((read, index) => names.push(safeIdentifier(read.name, `value${index + 1}`)));
    (definition.functions || []).forEach(fn => names.push(safeIdentifier(fn.name, 'myFunction')));
    return names;
  }

  function ensureNameScope(definition, options) {
    if (options && options.nameScope) return options.nameScope;
    return createNameScope(definitionReservedNames(definition));
  }

  /* أقواس زائدة مثل ((( شرط ))) تُربك القراءة — نزيل الطبقات الفائضة فقط */
  function stripOuterParens(expression) {
    let value = String(expression == null ? '' : expression).trim();
    let guard = 0;
    while (value.startsWith('(') && value.endsWith(')') && guard < 20) {
      guard += 1;
      let depth = 0;
      let wraps = true;
      let quote = '';
      for (let position = 0; position < value.length; position += 1) {
        ...
        if (character === '(') depth += 1;
          ...
          depth -= 1;
          if (depth === 0 && position < value.length - 1) { wraps = false; break; }
        }
      }
  ...
  /* علامتا التتبّع: تعليقان يحيطان كود كل خطوة أثناء التوليد بوضع traceSteps،
     فتعرف الواجهة أي سطر كتبته أي بطاقة. تُنزعان قبل عرض الكود أو حفظه. */
  const STEP_MARK_OPEN = '/*@osoos-step:';
  const STEP_MARK_CLOSE = '*/';

  function stripStepMarkers(code) {
    return String(code || '')
      .split('\n')
      .filter(line => line.indexOf(STEP_MARK_OPEN) === -1)
      .join('\n');
  }

  /* يفصل الكود المولَّد بوضع التتبّع إلى: كود نظيف + خريطة «معرّف الخطوة ← سطورها».
     نمشي سطرًا سطرًا بمكدس، فتُنسب الأسطر للخطوة الأعمق دائمًا حتى مع التداخل. */
  function splitTracedCode(traced) {
    const lines = String(traced || '').split('\n');
    const steps = Object.create(null);
    const stack = [];
    const clean = [];
    lines.forEach(line => {
      const at = line.indexOf(STEP_MARK_OPEN);
      if (at !== -1) {
        const end = line.indexOf(STEP_MARK_CLOSE, at + STEP_MARK_OPEN.length);
        const token = end === -1 ? '' : line.slice(at + STEP_MARK_OPEN.length, end);
        if (token.charAt(0) === '/') stack.pop();
        else {
          stack.push(token);
          if (!steps[token]) steps[token] = [];
        }
        return;
      }
      clean.push(line);
      const owner = stack[stack.length - 1];
      /* السطر يُنسب للخطوة الأعمق فقط؛ الخطوة الأم ترى أسطرها الخاصة */
      if (owner && steps[owner]) steps[owner].push(line);
    });
    const code = clean.join('\n');
    const map = Object.create(null);
    Object.keys(steps).forEach(id => {
      /* نزيل الإزاحة المشتركة حتى يبدأ السطر من أول عمود داخل البطاقة */
      const body = steps[id].filter(line => line.trim() !== '');
      const indentSize = body.reduce((min, line) => {
        const spaces = line.match(/^[ \t]*/)[0].length;
        return spaces < min ? spaces : min;
      }, Infinity);
      map[id] = body.map(line => line.slice(Number.isFinite(indentSize) ? indentSize : 0)).join('\n');
    });
    return { code: code, steps: map };
  }

  /* يحيط كتلة كود بعلامتَي خطوة — تُستعمل لمرحلة التعريف (الصناديق والوصفات)
     تمامًا كما تُستعمل داخل الإجراءات، فيصير لكل صفٍّ في الواجهة سطره الحقيقي. */
  function markStep(id, code, traceSteps) {
    if (!traceSteps || !text(code).trim()) return code;
    return `${STEP_MARK_OPEN}${id}${STEP_MARK_CLOSE}\n${code}\n${STEP_MARK_OPEN}/${id}${STEP_MARK_CLOSE}`;
  }

  /* حاوية state مرآة للمتغيرات؛ إن لم يقرأها أحد فهي ضجيج بصري خالص */
  function pruneStateMirrors(code) {
    const source = String(code || '');
    const mirror = /^[ \t]*state\.([A-Za-z_$][\w$]*) = \1;[ \t]*$/;
    const withoutMirrors = source.split('\n').filter(line => !mirror.test(line)).join('\n');
    if (/\bstate\s*[.[]/.test(withoutMirrors)) return source;
    return withoutMirrors
      .split('\n')
      .filter(line => !/^[ \t]*const state = Object\.create\(null\);[ \t]*$/.test(line))
      .join('\n');
  }
  ...
  /* ── قارئ «القيمة الأولية» ──
     المستخدم يكتب في خانة الصندوق بالطريقة التي يكتب بها JavaScript:
       ['a.jpg', 'b.jpg']        أو        { name: 'أحمد', age: 30 }
     وهذه ليست JSON صالحة (تنصيص مفرد، ومفاتيح بلا تنصيص)، فكان JSON.parse
     يفشل بصمت فتُفرَّغ القائمة إلى [] ويضيع ما كتبه المستخدم.
     هذا قارئ صغير يقرأ الحروف حرفًا حرفًا ويبني القيمة بنفسه — لا يشغّل أي
     كود، فلا يمكن لما يُكتب في الخانة أن يصير أمرًا يُنفَّذ. */
  function parseDataLiteral(input) {
    const src = String(input == null ? '' : input);
    let index = 0;
    const stop = () => { throw new Error('literal'); };

    function skipSpace() { while (index < src.length && /\s/.test(src[index])) index += 1; }
    ...
    function readString() {
      ...
      while (index < src.length) {
        const ch = src[index];
        if (ch === quote) { index += 1; return out; }
        if (ch === '\\') {
          ...
          else if (next === 'u') { out += String.fromCharCode(parseInt(src.slice(index, index + 4), 16) || 0); index += 4; }
          else out += next === undefined ? '' : next;
          continue;
        }
        /* قالب نصي بداخله ${...} يعني كودًا سيُنفَّذ لاحقًا — نرفضه */
        if (quote === '`' && ch === '$' && src[index + 1] === '{') stop();
        out += ch;
        index += 1;
      }
      return stop();
    ...
    function readNumber() {
      const match = /^-?(?:\d+\.?\d*|\.\d+)(?:[eE][-+]?\d+)?/.exec(src.slice(index));
      if (!match) return stop();
      index += match[0].length;
      return Number(match[0]);
    }

    function readWord(word, value) {
      if (src.slice(index, index + word.length) !== word) return undefined;
      index += word.length;
      return { value };
    }
    ...
    function readValue() {
      skipSpace();
      const ch = src[index];
      if (ch === undefined) return stop();
      if (ch === '[') return readArray();
      if (ch === '{') return readObject();
      if (ch === '"' || ch === "'" || ch === '`') return readString();
      const word = readWord('true', true) || readWord('false', false) || readWord('null', null);
      if (word) return word.value;
      return readNumber();
    }

    /* الفاصلة الزائدة قبل القوس الأخير مسموحة، لأن المستخدم يكتبها كثيرًا */
    function readList(closing, readItem) {
      index += 1;
      skipSpace();
      if (src[index] === closing) { index += 1; return true; }
      while (index < src.length) {
        readItem();
        skipSpace();
        if (src[index] === ',') {
    ...
    function readObject() {
      ...
      readList('}', () => {
        ...
        const ch = src[index];
        let key;
        if (ch === '"' || ch === "'" || ch === '`') key = readString();
        else {
          const match = /^[A-Za-z_$][A-Za-z0-9_$]*/.exec(src.slice(index));
          if (!match) return stop();
          index += match[0].length;
          key = match[0];
        }
        skipSpace();
        if (src[index] !== ':') return stop();
        index += 1;
        out[key] = readValue();
      });
      return out;
    ...
    try {
      if (!src.trim()) return { ok: false, value: undefined };
      const value = readValue();
      skipSpace();
      if (index !== src.length) return { ok: false, value: undefined };
  ...
  /* هل القيمة المكتوبة تصلح لنوع الصندوق؟ يستعمله المولّد والفاحص معًا
     حتى لا يقول أحدهما «صالحة» ويقول الآخر «غير صالحة». */
  function dataLiteralFor(rawValue, type) {
    const raw = text(rawValue).trim();
    const parsed = parseDataLiteral(raw || (type === 'Object' ? '{}' : '[]'));
    if (!parsed.ok) return { ok: false, value: undefined };
    if (type === 'Object') {
      const isPlain = parsed.value && typeof parsed.value === 'object' && !Array.isArray(parsed.value);
      return isPlain ? { ok: true, value: parsed.value } : { ok: false, value: undefined };
    }
    return Array.isArray(parsed.value) ? { ok: true, value: parsed.value } : { ok: false, value: undefined };
  }

  function variableInitialExpressionV2(variable) {
    const raw = text(variable.initialValue).trim();
    if (variable.type === 'Array') {
      const parsed = dataLiteralFor(raw, 'Array');
      return parsed.ok ? JSON.stringify(parsed.value) : '[]';
    }
    if (variable.type === 'Object') {
      const parsed = dataLiteralFor(raw, 'Object');
      return parsed.ok ? JSON.stringify(parsed.value) : '{}';
    }
    if (variable.type === 'Set') {
      const parsed = dataLiteralFor(raw, 'Set');
      return `new Set(${JSON.stringify(parsed.ok ? parsed.value : [])})`;
    }
  ...
  function generateVariablesV2(definition, scope, traceSteps) {
    ...
    return definition.state.map((variable, index) => ({ variable, index }))
      ...
      const line = `${declaration} ${name} = ${variableInitialExpressionV2(variable)};\nstate.${name} = ${name};`;
      /* معرّف الخطوة هو معرّف الصندوق نفسه، فتعرف الواجهة أي صفٍّ كتب أي سطر */
      return markStep(`decl:${variable.id || name}`, line, traceSteps);
    }).join('\n');
  }

  const READ_ELEMENT_TYPES = new Set(['element', 'inputValue', 'selectValue', 'textareaValue', 'checked', 'radioValue', 'textContent', 'innerText', 'innerHTML', 'attribute', 'dataAttribute', 'hasClass', 'styleProperty', 'childCount']);

  /* أسماء عناصر القراءة تُحسب مرة واحدة وتُخزَّن على النطاق،
     حتى يتطابق سطر البحث عن العنصر مع سطر قراءة قيمته. */
  function readElementNames(definition, scope) {
    if (scope.__readNames) return scope.__readNames;
    const names = new Map();
    definition.reads.filter(read => read.enabled !== false).sort((a, b) => a.order - b.order).forEach((read, index) => {
      if (!READ_ELEMENT_TYPES.has(read.type) || (read.type === 'radioValue' && !read.elementId)) return;
      const elementId = read.elementId || definition.sourceId;
      if (elementId && elementId === definition.sourceId) { names.set(index, 'sourceElement'); return; }
      names.set(index, elementCodeName(elementId, scope, 'field'));
    });
    scope.__readNames = names;
    return names;
  }

  function generateReadsSetupV2(definition, scope, lookups) {
    const names = readElementNames(definition, scope);
    const setup = [];
    const seen = new Set(['sourceElement']);
    definition.reads.filter(read => read.enabled !== false).sort((a, b) => a.order - b.order).forEach((read, index) => {
      const name = names.get(index);
      if (!name) return;
      const expression = `document.getElementById(${jsString(read.elementId || definition.sourceId)})`;
      /* الإجراءات اللاحقة تعيد استخدام نفس الثابت بدل بحث ثانٍ عن العنصر */
      if (lookups instanceof Map && !lookups.has(expression)) lookups.set(expression, name);
      if (seen.has(name)) return;
      seen.add(name);
      setup.push(`const ${name} = ${expression};`);
    });
    return setup.join('\n');
  }
  ...
  function readExpressionV3(read, elementName, definition) {
    ...
    else if (read.type === 'element') expression = elementName;
  ...
  function generateReadsBodyV2(definition, scope) {
    const elementReadTypes = READ_ELEMENT_TYPES;
    const names = readElementNames(definition, scope);
    ...
    return definition.reads.filter(read => read.enabled !== false).sort((a, b) => a.order - b.order).map((read, index) => {
      ...
      const elementName = names.get(index) || 'sourceElement';
      ...
      if (elementReadTypes.has(read.type) && !(read.type === 'radioValue' && !read.elementId)) {
        if (elementName === 'sourceElement') return `const ${name} = ${expression};`;
  ...
  function conditionExpressionV2(condition, definition) {
    ...
    if (condition.operator === 'enabled') return `Boolean(${left} && !${left}.disabled)`;
  ...
  /* الأقواس الخارجية الزائدة تُربك القراءة: ((الشرط)) ← الشرط.
     النتيجة تُستعمل داخل if (...) و while (...) فالمعنى لا يتغير،
     والأقواس الداخلية التي تفصل && عن || تبقى كما هي. */
  function conditionsExpressionV2(definition) {
    return stripOuterParens(conditionsExpressionSourceV2(definition));
  }

  function conditionsExpressionSourceV2(definition) {
  ...
  function actionValueExpressionV3(action, definition, options) {
    ...
    if (source.kind === 'state') {
      /* الصندوق المُعلن أعلى الكود يُقرأ باسمه المجرّد: notesCount أوضح بكثير
         من state["notesCount"]، ولا نلجأ لصندوق state إلا لاسم غير معرَّف هنا. */
      const stateName = text(source.name || raw).trim();
      return declaredVariableV2(definition, stateName) ? stateName : `state[${jsString(stateName)}]`;
    }
    if (source.kind === 'element') {
      ...
      /* إن كان العنصر مُعلنًا فعلاً في سطر أعلى، نستعمل اسمه بدل بحث جديد */
      const lookups = options && options.elementLookups instanceof Map ? options.elementLookups : null;
      const declaredName = lookups ? lookups.get(element) : '';
      /* sourceElement مفحوص في أعلى الكود، فلا داعي لعلامة ?. عليه */
      if (declaredName === 'sourceElement') return `sourceElement.${property}`;
      return `${declaredName || element}?.${property}`;
    }
  ...
  /* هل هذا الاسم صندوق ذاكرة معلَن فعلاً أعلى الكود؟
     نستعمله لنكتب اسم الصندوق كما هو بدل الالتفاف حوله. */
  function declaredVariableV2(definition, name) {
    const clean = text(name).trim();
    if (!/^[a-zA-Z_$][\w$]*$/.test(clean)) return null;
    const boxes = Array.isArray(definition && definition.state) ? definition.state : [];
    return boxes.find(variable => variable && variable.enabled !== false && variable.name === clean) || null;
  }

  /* نص مكتوب صراحةً لا يحتاج تغليفًا بـ String()، فـ String("أهلاً") ضجيج للقارئ */
  function stringExpressionV2(expression, definition) {
    const raw = text(expression).trim();
    if (/^"(?:[^"\\]|\\.)*"$/.test(raw) || /^'(?:[^'\\]|\\.)*'$/.test(raw)) return raw;
    /* صندوق من نوع String قيمته نص دائمًا، فالتغليف حوله لا يضيف شيئًا */
    const declared = declaredVariableV2(definition, raw);
    if (declared && declared.type === 'String') return raw;
    return `String(${raw})`;
  }

  /* رقم مكتوب صراحةً يبقى رقمًا: slice(0, 5) أوضح من slice(Number(0), Number(5)) */
  function numberExpressionV2(value, fallback) {
    const raw = text(value).trim();
    if (!raw) return String(fallback === undefined ? 0 : fallback);
    if (/^-?\d+(?:\.\d+)?$/.test(raw)) return raw;
    return `Number(${raw})`;
  }

  /* سطر النتيجة: إن كان الاسم صندوق ذاكرة معرَّفًا نُسنِد إليه فقط،
     وإلا نُعلنه هنا بـ const كما يفعل المبرمج عند أول استعمال للقيمة. */
  function resultLineV2(rawName, expression, definition, fallback) {
    const name = safeIdentifier(rawName, fallback || 'result');
    const declared = Boolean(declaredVariableV2(definition, name));
    return `${declared ? '' : 'const '}${name} = ${expression};\nstate.${name} = ${name};`;
  }

  /* شرط التصفية بلا كود: المستخدم يختار عملية وقيمة، ونحن نكتب السطر */
  /* قيمة يكتبها المستخدم في خانة مقارنة: إن كانت اسم صندوق معرَّف نكتبها
     كما هي ليقارن بالقيمة التي بداخله، وإلا فهي نصّ أو رقم عاديّ. */
  function valueOrBoxExpressionV2(raw, definition, fallback) {
    const clean = text(raw).trim();
    if (clean && declaredVariableV2(definition, clean)) return clean;
    return fallbackExpression(raw, fallback);
  }

  function arrayPredicateV2(operator, compareValue, definition) {
    const compare = valueOrBoxExpressionV2(compareValue, definition, "''");
    if (operator === 'notIncludes') return `!${stringExpressionV2('item')}.includes(${compare})`;
    if (operator === 'includes') return `${stringExpressionV2('item')}.includes(${compare})`;
    if (operator === 'startsWith') return `String(item).startsWith(${compare})`;
    if (operator === 'endsWith') return `String(item).endsWith(${compare})`;
    if (operator === 'isEmpty') return `String(item).trim() === ''`;
    if (operator === 'notEmpty') return `String(item).trim() !== ''`;
    if (operator === '===' || operator === '!==') return `item ${operator} ${compare}`;
    if (['>', '<', '>=', '<='].includes(operator)) return `Number(item) ${operator} ${compare}`;
    return 'Boolean(item)';
  }

  /* تحويل كل عنصر: نفس فكرة map لكن باختيار من قائمة بدل كتابة دالة */
  function arrayTransformV2(mode, template) {
    if (mode === 'upper') return 'String(item).toUpperCase()';
    if (mode === 'lower') return 'String(item).toLowerCase()';
    if (mode === 'trim') return 'String(item).trim()';
    if (mode === 'number') return 'Number(item) || 0';
    if (mode === 'template') return advancedTemplateExpression(template, 'item');
    return 'String(item)';
  }
  ...
  function generateActionBodyV2(action, targetName, definition, options) {
    ...
    /* الخصائص القياسية تُكتب كإسناد مباشر: img.src = ... لا setAttribute('src', ...).
       المتصفّح يحوّل القيمة إلى نصّ من تلقاء نفسه، فالسطران متطابقان في العمل
       ومختلفان في القراءة — وهذا ما يكتبه المبرمج بيده. setAttribute تبقى
       للسمات الحرّة (data-* و aria-*) التي لا خاصية جاهزة لها. */
    if (action.type === 'setSrc') {
      let code = `${targetName}.src = ${value};`;
      if (params.alt) {
        const altVal = valueExpressionV2(params.alt, params.altType || 'literal', definition);
        code += `\n  ${targetName}.alt = ${altVal};`;
      }
      return code;
    }
    if (action.type === 'setHref') {
      let code = `${targetName}.href = ${value};`;
      if (params.text) {
        const textVal = valueExpressionV2(params.text, params.textType || 'literal', definition);
        code += `\n  ${targetName}.textContent = ${textVal};`;
      }
      if (params.target) {
        const targetVal = valueExpressionV2(params.target, params.targetType || 'literal', definition);
        code += `\n  ${targetName}.target = ${targetVal};`;
      }
      if (params.rel) {
        const relVal = valueExpressionV2(params.rel, params.relType || 'literal', definition);
        code += `\n  ${targetName}.rel = ${relVal};`;
      }
      return code;
    }
    if (action.type === 'setLinkText') return `${targetName}.textContent = ${value};`;
    if (action.type === 'setTarget') return `${targetName}.target = ${value};`;
    if (action.type === 'setRel') return `${targetName}.rel = ${value};`;
    const propertyActions = { setAlt: 'alt', setPlaceholder: 'placeholder' };
    if (propertyActions[action.type]) return `${targetName}.${propertyActions[action.type]} = ${value};`;
    ...
    if (action.type === 'incrementVariable' || action.type === 'decrementVariable') {
      ...
      const down = action.type === 'decrementVariable';
      /* «دُر داخل قائمة»: بعد آخر عنصر نعود إلى الأول. هذا هو سطر % الذي
         يكتبه المبرمج بيده؛ وفي النزول نضيف الطول أولًا حتى لا يخرج رقم سالب. */
      const cycle = safeIdentifier(params.cycleArray, '');
      let code;
      if (cycle && declaredVariableV2(definition, cycle)) {
        code = down
          ? `${name} = (${name} - ${step} + ${cycle}.length) % ${cycle}.length;`
          : `${name} = (${name} + ${step}) % ${cycle}.length;`;
        code += `\nstate.${name} = ${name};`;
      } else {
        code = `${name} ${down ? '-=' : '+='} ${step};\nstate.${name} = ${name};`;
      }
    ...
    /* ===== عمليات القوائم: كل بطاقة تكتب سطر JavaScript واحدًا كما يكتبه المبرمج ===== */
    if (/^array(Pop|Shift|Unshift|Clear|Sort|Reverse|Unique|Join|Slice|Filter|Map|Length|Sum|IndexOf|Item)$/.test(action.type)) {
      const name = safeIdentifier(params.arrayName || params.variableName, 'items');
      const mirror = `\nstate.${name} = ${name};`;
      const operation = action.type.slice('array'.length);
      const wanted = text(params.resultName).trim();

      /* أولًا: عمليات تعدّل القائمة نفسها ولا تنتج قيمة جديدة */
      if (operation === 'Pop' || operation === 'Shift') {
        const method = operation === 'Pop' ? 'pop' : 'shift';
        if (!wanted) return `${name}.${method}();${mirror}`;
        return `${resultLineV2(wanted, `${name}.${method}()`, definition, 'removedItem')}${mirror}`;
      }
      if (operation === 'Unshift') return `${name}.unshift(${value});${mirror}`;
      if (operation === 'Clear') return `${name} = [];${mirror}`;
      if (operation === 'Reverse') return `${name}.reverse();${mirror}`;
      if (operation === 'Sort') {
        const [first, second] = text(params.direction) === 'desc' ? ['b', 'a'] : ['a', 'b'];
        const comparator = params.numeric === true || text(params.numeric) === 'number'
          ? `Number(${first}) - Number(${second})`
          : `String(${first}).localeCompare(String(${second}))`;
        return `${name}.sort((a, b) => ${comparator});${mirror}`;
      }

      /* ثانيًا: عمليات تُنتج قيمة جديدة وتترك القائمة الأصلية كما هي */
      if (operation === 'Unique') return resultLineV2(wanted || `${name}Unique`, `Array.from(new Set(${name}))`, definition, 'uniqueItems');
      if (operation === 'Join') {
        const separator = Object.prototype.hasOwnProperty.call(params, 'separator') ? params.separator : '، ';
        return resultLineV2(wanted || `${name}Text`, `${name}.join(${jsString(separator)})`, definition, 'joinedText');
      }
      if (operation === 'Slice') {
        const start = numberExpressionV2(params.start, 0);
        const args = text(params.end).trim() ? `${start}, ${numberExpressionV2(params.end, 0)}` : start;
        return resultLineV2(wanted || `${name}Part`, `${name}.slice(${args})`, definition, 'itemsPart');
      }
      if (operation === 'Filter') {
        const predicate = arrayPredicateV2(text(params.operator, 'includes'), params.compareValue, definition);
        return resultLineV2(wanted || `${name}Filtered`, `${name}.filter(item => ${predicate})`, definition, 'filteredItems');
      }
      if (operation === 'Map') {
        const transform = arrayTransformV2(text(params.transform, 'text'), params.template);
        return resultLineV2(wanted || `${name}Mapped`, `${name}.map(item => ${transform})`, definition, 'mappedItems');
      }
      if (operation === 'Item') {
        /* الموضع إمّا رقم مكتوب وإمّا اسم صندوق يحمل الرقم. اسم الصندوق
           يُكتب كما هو ليقرأ المستخدم slides[slideIndex] كما يكتبها المبرمج،
           لا slides[Number(slideIndex)]. */
        const rawIndex = text(params.index).trim();
        const index = declaredVariableV2(definition, rawIndex) ? rawIndex : numberExpressionV2(rawIndex, 0);
        return resultLineV2(wanted || `${name}Item`, `${name}[${index}]`, definition, 'currentItem');
      }
      if (operation === 'Length') return resultLineV2(wanted || `${name}Count`, `${name}.length`, definition, 'itemsCount');
      if (operation === 'Sum') return resultLineV2(wanted || `${name}Total`, `${name}.reduce((total, item) => total + (Number(item) || 0), 0)`, definition, 'itemsTotal');
      return resultLineV2(wanted || `${name}Index`, `${name}.indexOf(${value})`, definition, 'itemIndex');
    }
    /* ===== عمليات النصوص: قص وتقسيم واستبدال… من مصدر قيمة واحد ===== */
    if (/^string(Slice|Split|Replace|Case|Trim|Length|Concat|Number)$/.test(action.type)) {
      const source = stringExpressionV2(value, definition);
      const wanted = text(params.resultName).trim();
      const operation = action.type.slice('string'.length);
      if (operation === 'Slice') {
        const start = numberExpressionV2(params.start, 0);
        const args = text(params.end).trim() ? `${start}, ${numberExpressionV2(params.end, 0)}` : start;
        return resultLineV2(wanted || 'textPart', `${source}.slice(${args})`, definition, 'textPart');
      }
      if (operation === 'Split') {
        const separator = Object.prototype.hasOwnProperty.call(params, 'separator') ? params.separator : ' ';
        return resultLineV2(wanted || 'textParts', `${source}.split(${jsString(separator)})`, definition, 'textParts');
      }
      if (operation === 'Replace') {
        /* replaceAll يبدّل كل التكرارات، وreplace يبدّل أول تكرار فقط */
        const method = params.all === false || text(params.all) === 'first' ? 'replace' : 'replaceAll';
        return resultLineV2(wanted || 'newText', `${source}.${method}(${jsString(text(params.search))}, ${jsString(text(params.replacement))})`, definition, 'newText');
      }
      if (operation === 'Case') {
        const method = text(params.mode) === 'lower' ? 'toLowerCase' : 'toUpperCase';
        return resultLineV2(wanted || 'newText', `${source}.${method}()`, definition, 'newText');
      }
      if (operation === 'Trim') return resultLineV2(wanted || 'cleanText', `${source}.trim()`, definition, 'cleanText');
      if (operation === 'Length') return resultLineV2(wanted || 'textLength', `${source}.length`, definition, 'textLength');
      if (operation === 'Concat') {
        const separator = text(params.separator);
        /* لا شيء يُضاف؟ إذن الناتج هو النص نفسه، ولا داعي لكتابة «+ ''» يربك القارئ */
        const hasSuffix = text(params.suffix).trim() !== '';
        const suffix = fallbackExpression(params.suffix, "''");
        let joined = source;
        if (hasSuffix || separator) {
          joined = separator ? `${source} + ${jsString(separator)} + ${suffix}` : `${source} + ${suffix}`;
        }
        return resultLineV2(wanted || 'fullText', joined, definition, 'fullText');
      }
      return resultLineV2(wanted || 'numberValue', `Number(${value}) || 0`, definition, 'numberValue');
    }
    if (/^(local|session)Storage(Set|Get|Remove)$/.test(action.type)) {
      ...
      /* «قيمة بديلة» يكتبها المستخدم ككلمة عادية مثل light، وكان advancedExpression
         يمرّرها كما هي فتصير اسم متغيّر غير موجود → ReferenceError.
         fallbackExpression يضع علامات التنصيص حول الكلمة ويترك الأرقام
         والأقواس كما هي. */
      const fallback = fallbackExpression(params.fallback, "''");
      const parse = params.json ? `(() => { try { return JSON.parse(actionStoredRaw); } catch (error) { return ${fallback}; } })()` : `actionStoredRaw`;
      let code = `const ${storage} = ${generatedStorageAccessor(property)};\nconst actionStoredRaw = ${storage} ? ${storage}.getItem(${key}) : null;\n${resultLineV2(result, `actionStoredRaw === null ? ${fallback} : ${parse}`, definition, 'storedValue')}`;
    ...
    /* الكونسول: عنوان اختياري قبل القيمة ليعرف المستخدم أي سطر طبع ماذا */
    if (action.type === 'consoleLog' || action.type === 'log') {
      const level = ['log', 'info', 'warn', 'error'].includes(text(params.level)) ? params.level : 'log';
      const label = text(params.label).trim();
      return label ? `console.${level}(${jsString(label)}, ${value});` : `console.${level}(${value});`;
    }
    if (action.type === 'consoleTable') return `console.table(${safeIdentifier(params.arrayName || params.variableName, 'items')});`;
    if (action.type === 'consoleClear') return 'console.clear();';
    if (action.type === 'confirm' || action.type === 'prompt') {
      ...
      return resultLineV2(
        result,
        `typeof window.${method} === 'function' ? window.${method}(${args}) : ${method === 'confirm' ? 'false' : 'null'}`,
        definition,
        method === 'confirm' ? 'confirmed' : 'answer'
      );
    }
    ...
    if (action.type === 'setTimeout' || action.type === 'setInterval' || action.type === 'delayedAction') {
      ...
      return action.type === 'delayedAction'
        ? `${expression};`
        : resultLineV2(resultName, expression, definition, action.type === 'setInterval' ? 'intervalId' : 'timeoutId');
    }
    ...
    if (action.type === 'callFunction') {
      ...
      if (params.resultName) {
        ...
        if (params.await && options && options.asyncAllowed) return resultLineV2(resultName, `await Promise.resolve(${invocation})`, definition, 'functionResult');
        if (params.await) return `Promise.resolve(${invocation}).then(value => { state.${resultName} = value; });`;
        return resultLineV2(resultName, invocation, definition, 'functionResult');
      }
      return params.await && options && options.asyncAllowed ? `await Promise.resolve(${invocation});` : `${invocation};`;
    }
    /* بطاقة تُترجَم إلى كود جاهز تترك علامة في أول سطر لتُعاد قراءتها عند
       التعديل. العلامة تخص المحرّر لا القارئ، فتُنزع من الملف الناتج. */
    const custom = text(action.value || params.code, '// اكتب الكود المخصص هنا')
      .replace(/^\/\/ osoos-demo:[^\n]*\n?/, '');
    /* الكود المخصص يُكتب في الواجهة باسم targetElement؛ هنا يأخذ الاسم الحقيقي
       للعنصر بدل سطرَي إسناد وسيطين لا يقرأهما أحد. */
    if (!targetName) return custom;
    return custom.replace(/\b(?:targetElement|actionTarget)\b/g, targetName);
  }
  ...
  const TARGETLESS_ACTIONS = new Set(['branch', 'loop', 'setVariable', 'localStorageSet', 'localStorageRemove', 'sessionStorageSet', 'sessionStorageRemove', 'alert', 'confirm', 'prompt', 'openUrl', 'redirect', 'clipboard', 'print', 'setTimeout', 'setInterval', 'clearInterval', 'delayedAction', 'callFunction', 'log']);
  /* بطاقات القوائم والنصوص والكونسول تعمل على قيم في الذاكرة لا على عناصر الصفحة،
     فلا تحتاج هدفًا ولا حارس if حوله. مصدرها واحد مع ACTION_SCHEMAS أعلاه. */
  Object.keys(E1_ACTION_TYPES).forEach(type => {
    if (/^(array|string|console)[A-Z]/.test(type)) TARGETLESS_ACTIONS.add(type);
  });

  /* لواحق تصف علاقة الهدف بالعنصر الأساسي: parent ← ‎…Parent */
  const TARGET_KIND_SUFFIX = {
    parent: 'Parent', firstChild: 'FirstChild', child: 'FirstChild', lastChild: 'LastChild',
    children: 'Children', next: 'Next', previous: 'Previous', closest: 'Closest', querySelector: 'Inner'
  };

  function actionTargetNameSeed(action, definition) {
    const kind = action.target ? (action.target.kind || action.target.targetType) : '';
    const suffix = TARGET_KIND_SUFFIX[kind] || '';
    const baseId = action.targetId
      || (action.target && (action.target.id || action.target.baseId))
      || definition.targetId
      || '';
    if (!suffix) return baseId;
    const camel = camelFromId(baseId);
    return camel ? camel + suffix : '';
  }

  /* أهداف لا تحتاج بحثًا ولا حارسًا: معرَّفة أصلاً وغير قابلة لأن تكون null */
  const DIRECT_TARGET_EXPRESSIONS = new Set(['sourceElement', 'document', 'window']);

  function generateActionsV2(definition, actions, options) {
    const runtimeOptions = options || {};
    const scope = ensureNameScope(definition, runtimeOptions);
    /* نسخة من جدول العناصر المعلنة: نرث ما أُعلن فوقنا ولا نُسرّب
       ما نعلنه داخل كتلة (تكرار أو فرع) إلى ما بعدها. */
    const inherited = runtimeOptions.elementLookups instanceof Map ? runtimeOptions.elementLookups : null;
    const lookups = new Map(inherited || []);
    const nested = Object.assign({}, runtimeOptions, { nameScope: scope, elementLookups: lookups });

    const prepared = (actions || definition.actions).map((action, index) => ({ action, index }))
      .filter(item => item.action.enabled !== false)
      .sort((a, b) => a.action.order - b.action.order || a.index - b.index)
      .map(item => {
        const action = item.action;
        /* عدّاد لا يُعرَض لا يلمس أي عنصر، فحارس «if (theImage)» حوله
           تداخل بلا فائدة يصعّب القراءة. */
        const silentCounter = (action.type === 'incrementVariable' || action.type === 'decrementVariable')
          && action.params && action.params.display === false;
        const needsTarget = !TARGETLESS_ACTIONS.has(action.type)
          && !silentCounter
          && !(action.type === 'custom' && !action.targetId)
          && !((action.type === 'localStorageGet' || action.type === 'sessionStorageGet') && !action.targetId);
        return { action, needsTarget, expression: needsTarget ? targetExpressionV2(action, definition, nested) : '' };
      });

    /* ترقيم الخطوات بتعليق عربي قصير — يظهر فقط حين تتعدد الخطوات */
    const numbered = prepared.length > 1;
    /* اسم البطاقة كما يراه المستخدم على الشاشة. البطاقات التي تُترجَم إلى
       كود جاهز تحمل علامة في أول سطرها، فنقرؤها لنكتب اسمها الحقيقي بدل
       «تشغيل كود مخصص» الذي لا يصف ما فعله المستخدم فعلًا. */
    const stepLabelV2 = action => {
      if (action.type === 'custom') {
        const marker = text(action.params && action.params.code).match(/^\/\/ osoos-demo:(\w+)/);
        if (marker && E1_ACTION_TYPES[marker[1]]) return E1_ACTION_TYPES[marker[1]];
      }
      return E1_ACTION_TYPES[action.type] || 'خطوة';
    };
    /* وضع التتبّع: نحيط كود كل خطوة بعلامتين حتى تعرف الواجهة أي سطر
       كتبته أي بطاقة، فتعرضه على البطاقة نفسها بالترتيب الحقيقي للملف.
       العلامتان تعليقان يُنزعان قبل عرض الكود النهائي. */
    const traceSteps = Boolean(runtimeOptions.traceSteps);
    const withStep = (action, targetName, position) => {
      const body = generateActionBodyV2(action, targetName, definition, nested);
      const labelled = numbered ? `// ${position}. ${stepLabelV2(action)}\n${body}` : body;
      if (!traceSteps) return labelled;
      const id = text(action.id) || `step-${position}`;
      return `${STEP_MARK_OPEN}${id}${STEP_MARK_CLOSE}\n${labelled}\n${STEP_MARK_OPEN}/${id}${STEP_MARK_CLOSE}`;
    };
    ...
    while (cursor < prepared.length) {
      ...
      if (!entry.needsTarget || DIRECT_TARGET_EXPRESSIONS.has(entry.expression)) {
        lines.push(withStep(entry.action, entry.needsTarget ? entry.expression : '', cursor + 1));
        cursor += 1;
        continue;
      }
      /* خطوات متتالية على نفس العنصر تتشارك حارسًا واحدًا بدل تكراره */
      const run = [entry];
      let next = cursor + 1;
      while (next < prepared.length && prepared[next].needsTarget && prepared[next].expression === entry.expression) {
        run.push(prepared[next]);
        next += 1;
      }
      let name = lookups.get(entry.expression);
      const alreadyDeclared = Boolean(name);
      if (!name) {
        name = elementCodeName(actionTargetNameSeed(entry.action, definition), scope, 'element');
        lookups.set(entry.expression, name);
      }
      const inner = run.map((item, offset) => withStep(item.action, name, cursor + offset + 1)).join('\n');
      /* العنصر المصدر مفحوص أصلًا في أعلى الكود (نخرج مبكرًا لو كان مفقودًا)
         فحارس ثانٍ حوله تكرار بلا فائدة يزيد التداخل ويصعّب القراءة. */
      if (DIRECT_TARGET_EXPRESSIONS.has(name)) {
        lines.push(inner);
        cursor = next;
        continue;
      }
      /* سطر إحضار العنصر وحارسه يُنسبان لأول بطاقة احتاجته، لا للبوابة فوقها:
         هكذا يقرأ المستخدم على البطاقة نفسها من أين جاء الاسم الذي تستعمله. */
      let block = alreadyDeclared ? '' : `const ${name} = ${entry.expression};\n`;
      block += `if (${name}) {\n${indent(inner, 2)}\n}`;
      lines.push(markStep(text(entry.action.id) || `step-${cursor + 1}`, block, traceSteps));
      cursor = next;
    }
    return lines.join('\n');
  }
  ...
  function generateFunctionsV3(definition, options, standaloneState) {
    ...
    return definition.functions.map((fn, index) => ({ fn, index }))
      ...
      .map(item => markStep(
        `decl:${item.fn.id || item.fn.name}`,
        generateOneFunctionV3(definition, item.fn, item.index, options || {}, standaloneState),
        traceSteps
      ))
      .join('\n');
  }
  ...
  function generateExecutableV2(input, options) {
    ...
    /* نطاق أسماء واحد لكل قاعدة: يمنع تصادم أسماء العناصر مع المتغيرات والقراءات */
    const nameScope = createNameScope(definitionReservedNames(definition));
    const scopedOptions = Object.assign({}, options, { nameScope: nameScope });
    let handler = '';
    ...
    const traceSteps = Boolean(options && options.traceSteps);
    const eventVariables = generateVariablesV2(definition, 'insideEvent', traceSteps);
    if (eventVariables) handler += eventVariables + '\n';
    /* البحث عن عناصر القراءة لحظة الحدث: لا مراجع قديمة ولا تعطيل كامل عند غياب عنصر */
    const handlerLookups = new Map();
    /* العنصر المُشغِّل معرَّف ومفحوص في أعلى الملف باسم sourceElement، فأي قراءة
       منه داخل المعالج تستعمل الاسم مباشرة بدل بحث ثانٍ عن العنصر نفسه. */
    if (definition.sourceId) {
      handlerLookups.set(`document.getElementById(${jsString(definition.sourceId)})`, 'sourceElement');
    }
    const readSetup = generateReadsSetupV2(definition, nameScope, handlerLookups);
    const reads = generateReadsBodyV2(definition, nameScope);
    /* سطور القراءة صفٌّ واحد في قائمة التنفيذ: «اقرأ قيمة العنصر» قبل الشرط */
    const readBlock = markStep('run:reads', [readSetup, reads].filter(Boolean).join('\n'), traceSteps);
    if (readBlock) handler += readBlock + '\n';
    const advancedReads = generateAdvancedPhaseV3(definition, 'reads', scopedOptions);
    if (advancedReads) handler += advancedReads + '\n';
    const advancedConditions = generateAdvancedPhaseV3(definition, 'conditions', scopedOptions);
    if (advancedConditions) handler += advancedConditions + '\n';
    let advancedActionsCode = generateAdvancedPhaseV3(definition, 'actions', scopedOptions);
    ...
    const actionOptions = Object.assign({}, scopedOptions, { asyncAllowed: true, elementLookups: handlerLookups });
    const actions = [generateActionsV2(definition, null, actionOptions), advancedActionsCode].filter(Boolean).join('\n') || '// لا توجد إجراءات';
    const condition = stripOuterParens(conditionsExpressionV2(definition));
    handler += condition ? `if (${condition}) {\n${indent(actions, 2)}\n}` : actions;
    /* block scope حديث وواضح: يمنع تصادم أسماء أكثر من تفاعل من غير IIFE
       أو سجل عالمي أو AbortController لا يحتاجه ملف التصدير الثابت. */
    let code = '{\n';
    const eventUsesWindow = definition.event === 'load' || definition.event === 'resize' || (definition.event === 'scroll' && eventSettings.target === 'window');
    /* سطر إمساك العنصر صفٌّ مستقل في قائمة التعريف: «أمسك العنصر من الصفحة» */
    let sourceBlock = `const sourceElement = ${definition.sourceId ? `document.getElementById(${jsString(definition.sourceId)})` : (eventUsesWindow ? '(document.documentElement || document.body || document)' : 'null')};`;
    code += indent(markStep('decl:source-element', sourceBlock, traceSteps), 2) + '\n';
    if (!eventUsesWindow) code += '  if (sourceElement) {\n';
    const bodyIndent = eventUsesWindow ? 2 : 4;
    const variables = generateVariablesV2(definition, 'outsideEvent', traceSteps);
    const advancedState = generateAdvancedPhaseV3(definition, 'state', scopedOptions);
    const functions = generateFunctionsV3(definition, scopedOptions, false);
    ...
    if (stateIsUsed) code += `${' '.repeat(bodyIndent)}const state = Object.create(null);\n`;
    if (variables) code += indent(variables, bodyIndent) + '\n';
    if (advancedState) code += indent(advancedState, bodyIndent) + '\n';
    if (functions) code += indent(functions, bodyIndent) + '\n';
    ...
    /* async وخيارات المستمع لا تُكتب إلا عند الحاجة الفعلية. */
    ...
    const listenerOptionsSource = listenerOptions.length ? `, { ${listenerOptions.join(', ')} }` : '';
    /* غلاف الحدث نفسه صفٌّ في قائمة التنفيذ: «عند النقر…» — والخطوات بداخله */
    if (traceSteps) code += `${' '.repeat(bodyIndent)}${STEP_MARK_OPEN}run:trigger${STEP_MARK_CLOSE}\n`;
    code += `${' '.repeat(bodyIndent)}${eventSource}.addEventListener(${jsString(eventName)}, ${handlerIsAsync ? 'async ' : ''}(event) => {\n`;
    code += indent(handler, bodyIndent + 2) + `\n${' '.repeat(bodyIndent)}}${listenerOptionsSource});\n`;
    if (traceSteps) code += `${' '.repeat(bodyIndent)}${STEP_MARK_OPEN}/run:trigger${STEP_MARK_CLOSE}\n`;
    if (!eventUsesWindow) code += '  }\n';
    code += '}';
    return pruneStateMirrors(code);
  }

  /* الكود نفسه، لكن معه خريطة تقول: هذه السطور كتبتها هذه البطاقة.
     تستعملها الواجهة لتعرض على كل بطاقة سطرها الحقيقي بترتيب الملف. */
  function generateTracedCode(input, options) {
    const traced = generateExecutableV2(input, Object.assign({}, options, { traceSteps: true }));
    const split = splitTracedCode(traced);
    return { code: stripStepMarkers(split.code), steps: split.steps };
  }
  ...
  function generateBlockV2(input, options) {
    ...
    return `// @osoos-interaction id="${id}"\n` +
      generateExecutableV2(definition, options) + '\n' +
      `// @end-osoos-interaction id="${id}"`;
  }

  function parseVisualLinksV2(customJS, storedDefinitions) {
    const lines = text(customJS).split(/\r?\n/);
    const links = [];
    const definitions = new Map((Array.isArray(storedDefinitions) ? storedDefinitions : [])
      .filter(Boolean)
      .map(definition => {
        const normalized = normalizeDefinitionV2(definition);
        return [normalized.id, normalized];
      }));
    let current = null;
    for (let index = 0; index < lines.length; index += 1) {
      const trimmed = lines[index].trim();
      if (trimmed.indexOf('// @osoos-interaction') === 0) {
        const match = trimmed.match(/id="([^"]+)"/);
        if (match) current = {
          id: match[1],
          startIndex: index,
      ...
      if (!current) continue;
      current.rawLines.push(lines[index]);
      if (current.modern && trimmed.indexOf('// @end-osoos-interaction') === 0) {
        const endMatch = trimmed.match(/id="([^"]+)"/);
        if (!endMatch || endMatch[1] !== current.id) continue;
        const stored = definitions.get(current.id);
        if (stored) {
          const definition = normalizeDefinitionV2(stored);
          definition.startIndex = current.startIndex;
  ...
  function validateDefinitionV2(input) {
    ...
    definition.state.forEach((variable, index) => {
      ...
      if (variable.type === 'Array' || variable.type === 'Object' || variable.type === 'Set') {
        /* نفس القارئ الذي يستعمله المولّد، فلا يختلف حكم الفحص عن حكم التوليد */
        if (!dataLiteralFor(variable.initialValue, variable.type).ok) {
          errors.push(`القيمة الأولية للمتغير ${variable.name} غير صالحة لنوع ${variable.type}.`);
        }
      }
    });
    ...
    const validateActions = (actionsList, pathPrefix) => {
      actionsList.forEach((action, index) => {
        ...
        /* كل بطاقة تعمل على متغير (عدّاد أو قائمة) تحتاج صندوق ذاكرة معرَّفًا أولاً،
           وإلا كان الكود المولَّد يشير إلى اسم غير موجود. */
        if (/^(incrementVariable|decrementVariable|toggleBoolean|setVariable|array[A-Z]\w*|consoleTable)$/.test(action.type)) {
        ...
        /* «دُر داخل قائمة» يقرأ طول قائمة: إن لم تكن معرَّفة يسقط اللف بصمت،
           فنقولها للمستخدم بدل أن يكتشف بنفسه أن العدّاد لا يعود للبداية. */
        if (/^(incrementVariable|decrementVariable)$/.test(action.type) && text(action.params.cycleArray).trim()) {
          const cycle = safeIdentifier(action.params.cycleArray, '');
          if (!definition.state.some(variable => variable.name === cycle && variable.type === 'Array')) {
            errors.push(`قائمة الدوران ${text(action.params.cycleArray)} في ${label} غير معرَّفة كصندوق من نوع قائمة.`);
          }
        }
