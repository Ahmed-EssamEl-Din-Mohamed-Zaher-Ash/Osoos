...
class OsoosStyleEngine {
  ...
  rewriteCssSelectorPreludes(css, options = {}) {
    ...
    const rewritePrelude = prelude => {
      ...
      const changed = this.rewriteSelectorReferences(protectedCore, options)
        // The control-character sentinel cannot collide with valid CSS text.
  ...
  commitToEditor() {
    ...
    if (this.app.customStyleTag) {
      const unmanagedCss = this.stripManagedBlock(nextCss);
      this.app.customStyleTag.textContent = typeof this.app.scopeCssForCanvas === 'function'
        ? this.app.scopeCssForCanvas(unmanagedCss)
        : unmanagedCss;
    }
    this.updatePreview();
...
export { OsoosStyleEngine };

export const styleEngineService = Object.freeze({
  OsoosStyleEngine
});
